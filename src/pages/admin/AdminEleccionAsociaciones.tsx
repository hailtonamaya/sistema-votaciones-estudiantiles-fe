...
export default function AdminEleccionAsociaciones() {
  ...
  const goToNext = () => navigate(`/admin/elecciones/candidatos?election_id=${selectedId}`)

  return (
    <AdminLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: BRAND }}>Asociaciones</h1>
        <p className="mt-0.5 text-sm text-gray-500">Registra las asociaciones que participarán en cada carrera.</p>
      </div>
        ...
        <>
          ...
          {selectedElection?.status === "open" && (
            <div className="mb-5 flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              ...
              Esta elección está <strong className="mx-1">activa</strong> — paúsala para poder editar las asociaciones.
            </div>
          )}
          {(selectedElection?.status === "closed" || selectedElection?.status === "cancelled") && (
            <div className="mb-5 flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
              ...
              Esta elección está <strong className="mx-1">{selectedElection.status === "closed" ? "finalizada" : "archivada"}</strong> — las asociaciones son de solo lectura.
            </div>
          )}
