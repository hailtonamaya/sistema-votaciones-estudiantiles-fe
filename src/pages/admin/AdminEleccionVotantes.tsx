...
export default function AdminEleccionVotantes() {
  ...
  const goToPrev = () => navigate(`/admin/elecciones/candidatos?election_id=${selectedId}`)
  const goToNext = () => navigate(`/admin/elecciones/revision?election_id=${selectedId}`)

  return (
    <AdminLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: BRAND }}>Votantes</h1>
        <p className="mt-0.5 text-sm text-gray-500">Gestiona el padrón de votantes habilitados para la elección.</p>
      </div>
      ...
      {!selectedId ? (
        <div className="flex flex-col items-center justify-center rounded-2xl bg-white py-20 px-8 shadow-sm text-center">
          ...
          <p className="max-w-sm text-sm text-gray-500">
            Elige una elección en el selector de arriba para ver y gestionar su padrón de votantes.
          </p>
        </div>
      ) : (
        <>
          ...
          {selectedElection?.status === "open" && (
            <div className="mb-5 flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              ...
              Esta elección está <strong className="mx-1">activa</strong> — paúsala para poder editar el padrón de votantes.
            </div>
          )}
          {(selectedElection?.status === "closed" || selectedElection?.status === "cancelled") && (
            <div className="mb-5 flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
              ...
              Esta elección está <strong className="mx-1">{selectedElection.status === "closed" ? "finalizada" : "archivada"}</strong> — el padrón es de solo lectura.
            </div>
          )}
